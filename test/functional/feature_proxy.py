...
"""Test bltgd with different proxy configuration.

Test plan:
- Start bltgd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bltgd side:
...
- Create bltgds that connect to them
- Manipulate the bltgds using addnode (onetry) an observe effects
...
class ProxyTest(BltgTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: bltgd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
