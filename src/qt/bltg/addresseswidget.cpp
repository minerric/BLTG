...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto bltgAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(bltgAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(bltgAdd);
