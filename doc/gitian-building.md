...
*Setup instructions for a gitian build of BLTG using a VM or physical system.*
...
- [Building bltg Core](#building-bltg-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bltg-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bltg-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bltg-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bltg-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bltg-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bltg-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bltg-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bltg.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bltg-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bltg-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bltg-osx-*-build.assert
```
