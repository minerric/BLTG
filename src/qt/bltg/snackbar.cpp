...
#include "qt/bltg/forms/ui_snackbar.h"
#include "qt/bltg/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BLTGGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BLTGGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
