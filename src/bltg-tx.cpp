...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " bltg-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  bltg-tx [options] <hex-tx> [commands]  Update hex-encoded bltg transaction\n" +
                               "or:     bltg-tx [options] -create [commands]   Create hex-encoded bltg transaction\n" +
                               "\n";
