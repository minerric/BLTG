...
class ReorgStakeTest(BltgTestFramework):
    ...
    def check_money_supply(self, expected_bltg):
        # verify that nodes have the expected BLTG supply
        bltg_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(bltg_supply, [DecimalAmt(expected_bltg)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that BLTG supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check BLTG supply...")
