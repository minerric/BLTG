BLTG Core version *version* is now available from:  <https://github.com/Block-Logic-Technology-Group/bltg/releases>
...
```
Mainnet: Saturday, April 30, 2022 12:00:00 AM GMT
```
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/PIVX-Qt (on Mac) or bltgd/bltg-qt (on Linux).
...
Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zBLTG have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zBLTG with public spends version 4 ([PR #936](https://github.com/PIVX-Project/PIVX/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zBLTG), version 4 spends will also be public. A full technical writeup is available on the [PIVX Wiki](https://github.com/PIVX-Project/PIVX/wiki/CoinRandomnessSchnorrSignature).
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~bltg) repository, we are now offering a [Snap package](https://snapcraft.io/bltg-core) as quick way to install and update a PIVX wallet.
