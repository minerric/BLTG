...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t mNodeCoins = nMasternodeCount * 12000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZBLTGStake)
{
    ...
    if (nHeight <= 43200) {
        ...
        //When zBLTG is staked, masternode only gets 2 BLTG
        ret = 3 * COIN;
        if (isZBLTGStake)
            ret = 2 * COIN;
    }
...
int ActiveProtocol()
{
    // SPORK_14 was used for 70922 (v3.4), commented out now.
    //if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 is used for 70923 (v4.0+)
