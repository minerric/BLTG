...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BLTG.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZbltgSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZbltg(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzBLTGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBLTGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBLTGReset.connect(boost::bind(NotifyzBLTGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBLTGReset.disconnect(boost::bind(NotifyzBLTGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbltgTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
