...
CZBltgStake::CZBltgStake(const libzerocoin::CoinSpend& spend)
{
...
int CZBltgStake::GetChecksumHeightFromMint()
{
...
int CZBltgStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZBltgStake::GetChecksum()
{
    return nChecksum;
}

// The zBLTG block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZBltgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZBltgStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zBLTG staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZBltgStake::GetUniqueness()
{
    //The unique identifier for a zBLTG is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZBltgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZBltgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zBLTG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBLTGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBLTG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBLTG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBLTGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBLTG output", __func__);
...
bool CZBltgStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZBltgStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzBLTGTracker* zbltgTracker = pwallet->zbltgTracker.get();
    CMintMeta meta;
    if (!zbltgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbltgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!BLTG Stake
bool CBltgStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBltgStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}
...
CAmount CBltgStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CBltgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CBltgStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CBltgStake::GetUniqueness()
{
