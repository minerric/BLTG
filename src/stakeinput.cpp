...
CBltgStake* CBltgStake::NewBltgStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CBltgStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CBltgStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}
...
CTxIn CBltgStake::GetTxIn() const
{
...
bool CBltgStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
