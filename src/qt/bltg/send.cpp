    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("BLTG");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zBLTG");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BLTG) or private coins (zBLTG)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a BLTG address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BLTG");
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send BLTG");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zBLTG");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZbltg = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZbltg));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZbltg ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendBltg = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendBltg ? AskPassphraseDialog::Context::Send_BLTG : AskPassphraseDialog::Context::Send_zBLTG, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendBltg) ? send(recipients) : sendZbltg(recipients)) {
...
bool SendWidget::sendZbltg(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBLTG is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (!ZBltgControlDialog::setSelectedMints.empty()) {
    ...
    if (walletModel->sendZbltg(
        ...
        inform(tr("zBLTG transaction sent!"));
        ZBltgControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZBLTG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBLTG require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zBLTG transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isBLTG){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BLTG to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZBltgControlDialog *zBltgControl = new ZBltgControlDialog(this);
            zBltgControl->setModel(walletModel);
            zBltgControl->exec();
            ui->btnCoinControl->setActive(!ZBltgControlDialog::setSelectedMints.empty());
            zBltgControl->deleteLater();
        } else {
            inform(tr("You don't have any zBLTG in your balance to select."));
        }
    }
}
...
void SendWidget::onBLTGSelected(bool _isBLTG){
    isBLTG = _isBLTG;
    setCssProperty(coinIcon, _isBLTG ? "coin-icon-bltg" : "coin-icon-zbltg");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress bltgAdd = CBitcoinAddress(address.toStdString());
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bltgAdd.Get(), dialog->getLabel().toStdString(), "send")) {
